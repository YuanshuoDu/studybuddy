...
External HTTP ping service. The StudyBuddy API is considered "up"
...
2. **Create 3 monitors** — one per region so we catch regional
   ...
   | --- | --- | --- | --- |
   | `studybuddy-api-eu-west` | `https://api.studybuddy.app/api/v1/monitoring/liveness` | 1 min | Frankfurt probe |
   | `studybuddy-api-us-east` | same | 1 min | Virginia probe (catches EU-only outages) |
   | `studybuddy-api-ap-southeast` | same | 1 min | Singapore probe (catches EU + US-only outages) |
...
4. **Status page** (optional, free): Better Uptime can host a
   `status.studybuddy.app` page that shows public uptime. Worth
...
| Background jobs | Use the `studybuddy_background_job_last_run_timestamp` metric + R8 alert rule |
...
2. Check **Grafana** for the StudyBuddy API Overview dashboard. If
   `studybuddy_http_requests_in_flight` is climbing, the process
