...
This is the operator-facing entry point for the StudyBuddy monitoring
...
| --- | --- | --- | --- |
| R1 | `StudyBuddyHigh5xxRate` | critical | 5xx > 1% over 5m — real bug shipping errors |
| R2 | `StudyBuddyHighLatencyP99` | warning | p99 > 1s for 10m on any route |
| R3 | `StudyBuddyAuthFailureStorm` | warning | > 50 auth failures in 5m |
| R4 | `StudyBuddyRateLimitedHotRoute` | warning | > 1000 rate-limited req/5m on one route |
| R5 | `StudyBuddyRedisOutage` | critical | Redis error > 50% for 2m |
| R6 | `StudyBuddyContentSafetyDisabled` | warning | WeChat content-safety not configured |
| R7 | `StudyBuddyContentSafetyFailOpen` | warning | WeChat API fail-open > 5% for 10m |
| R8 | `StudyBuddyBackgroundJobStalled` | warning | A cron / worker has not run in 2h |
...
All metrics are prefixed with `studybuddy_` and live in
`server/src/lib/metrics.ts`. The curated set is:

| Metric | Type | Labels | Use |
| --- | --- | --- | --- |
| `studybuddy_http_requests_total` | counter | method, route, status_class | Throughput + error rate |
| `studybuddy_http_request_duration_seconds` | histogram | method, route, status_class | p50 / p95 / p99 latency |
| `studybuddy_http_requests_in_flight` | gauge | — | Concurrency / saturation |
| `studybuddy_auth_failures_total` | counter | route, reason | Brute force / token expiry detection |
| `studybuddy_rate_limited_total` | counter | route | Abuse detection |
| `studybuddy_module_events_total` | counter | module, event | Business KPIs (signups, activities) |
| `studybuddy_db_query_duration_seconds` | histogram | model, action | Slow query detection |
| `studybuddy_cache_ops_total` | counter | op, result | Redis cache hit ratio |
| `studybuddy_content_check_total` | counter | result | WeChat content-safety effectiveness |
| `studybuddy_background_job_last_run_timestamp` | gauge | job | Stalled-job detection |

Plus the `prom-client` default Node.js metrics (CPU, memory, GC, etc.)
with the same `studybuddy_` prefix.
...
3. **No name collisions.** Every metric has the `studybuddy_`
   prefix. Sentry and the alert rules use the same names — search
   for `studybuddy_` in this repo and you'll see every metric.
