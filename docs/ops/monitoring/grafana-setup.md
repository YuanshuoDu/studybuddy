...
3. **Wire a Prometheus agent** (or use Grafana Agent / Alloy) to
   scrape `https://api.studybuddy.app/metrics` every 15s and remote-
...
4. **Verify** by visiting your Grafana → Explore → Prometheus →
   query `studybuddy_http_requests_total`. You should see a non-empty
...
### 1. StudyBuddy API Overview

The "first screen on call" — single 12-cell grid, refresh every 30s.

| Cell | Query |
| --- | --- |
| 5xx rate (last 5m) | `sum(rate(studybuddy_http_requests_total{status_class="5xx"}[5m]))` |
| 4xx rate | `sum(rate(studybuddy_http_requests_total{status_class="4xx"}[5m]))` |
| 2xx rate | `sum(rate(studybuddy_http_requests_total{status_class="2xx"}[5m]))` |
| p50 / p95 / p99 latency | `histogram_quantile(0.50, ...)` × 3 |
| Requests in flight | `studybuddy_http_requests_in_flight` |
| Cache hit ratio | `rate(studybuddy_cache_ops_total{result="hit"}[5m]) / rate(studybuddy_cache_ops_total{op="get"}[5m])` |
| Top 10 routes by error rate | `topk(10, sum by (route) (rate(studybuddy_http_requests_total{status_class="5xx"}[5m])))` |
| Top 10 routes by p99 latency | `topk(10, histogram_quantile(0.99, sum by (route, le) (rate(studybuddy_http_request_duration_seconds_bucket[10m]))))` |

### 2. StudyBuddy Auth + Abuse
...
| Cell | Query |
| --- | --- |
| 401 by reason | `sum by (reason) (rate(studybuddy_auth_failures_total{reason="unauthorized"}[5m]))` |
| 403 by reason | `sum by (reason) (rate(studybuddy_auth_failures_total{reason="forbidden"}[5m]))` |
| Rate-limited by route (top 10) | `topk(10, sum by (route) (rate(studybuddy_rate_limited_total[5m])))` |
| Refresh-token rotation | computed from `auth:refresh:*` Redis key TTL distribution |

### 3. StudyBuddy Business KPIs

What the PMs care about. Refresh every 5m.

| Cell | Query |
| --- | --- |
| Signups today / this week | `studybuddy_module_events_total{module="signup",event="created"}` |
| Activities created today | `studybuddy_module_events_total{module="activity",event="created"}` |
| Reviews created today | `studybuddy_module_events_total{module="review",event="created"}` |
...
3. For each of the 8 rules in `alert-rules.yaml`:
   - Name: `StudyBuddy*` (matches the YAML)
