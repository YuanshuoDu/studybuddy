...
# 4. 上传到自有下载页（studybuddy.app/download）
...
`com.studybuddy.app`，签名一致可以让用户后续从 Google Play 装的版本
和你直发的版本互认）。
...
大陆境内的 server 域名 `studybuddy.app` 走海外 DNS 解析，**不需要**
ICP 备案（你的 API server 在 AWS Frankfurt，DNS 在 Cloudflare）。但
如果你做 v1.1 国内落地页（studybuddy.cn / studybuddy.com.cn），那
...
gh release create v1.0.0 \
  build/app/outputs/flutter-apk/app-release.apk \
  --title "StudyBuddy v1.0.0 (国内 APK 直发)" \
  --notes-file .harness/release-notes-1.0.0.md
# → https://github.com/YuanshuoDu/studybuddy/releases/tag/v1.0.0
```

下载链接 = `https://github.com/YuanshuoDu/studybuddy/releases/download/v1.0.0/app-release.apk`
...
`https://studybuddy.app/download`（CN 域名）落地页：
...
1. **"下载完找不到文件"** → 默认下载目录是 `Download/`，但部分手机
   把 APK 放在 `Download/StudyBuddy/`，加进 FAQ
2. **"提示病毒风险"** → MIUI / EMUI 默认有"病毒扫描"，APK 第一次
   安装会弹"该应用可能存在风险"。需要在 FAQ 写明"StudyBuddy 是开源
   软件，源码在 github.com/YuanshuoDu/studybuddy"
...
- 500-5000：加 Vercel 落地页（`studybuddy.app/download`）
