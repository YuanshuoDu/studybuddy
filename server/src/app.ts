...
async function buildApp(): Promise<FastifyInstance> {
  ...
  await app.register(rateLimit, {
    ...
    errorResponseBuilder: () => ({
      type: 'https://studybuddy.example.com/errors/rate-limit',
  ...
  app.setErrorHandler((err, req, reply) => {
    ...
    if (err.validation) {
      return reply.code(400).send({
        type: 'https://studybuddy.example.com/errors/validation',
    ...
    return reply.code(status).send({
      type: err.code
        ? `https://studybuddy.example.com/errors/${String(err.code).toLowerCase()}`
        : 'https://studybuddy.example.com/errors/internal',
